...
from anibridge.providers.library.emby.webhook import EmbyWebhook
...
async def test_webhook_from_json_body() -> None:
    ...
    webhook = await EmbyWebhook.from_request(cast(Request, request))
...
async def test_webhook_invalid_form_payload_raises() -> None:
    ...
    with pytest.raises(ValueError, match="Invalid payload JSON"):
        await EmbyWebhook.from_request(cast(Request, request))
